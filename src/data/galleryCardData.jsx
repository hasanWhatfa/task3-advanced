const galleryCardData =[
    {
        filter:'Classrooms',
        imgsArr:[
            './assets/images/classroomZero.png',
            './assets/images/classroomOne.png',
            './assets/images/classroomTwo.png',
            './assets/images/classroomThree.png',
            './assets/images/classroomZero.png',
            './assets/images/classroomOne.png',
            './assets/images/classroomTwo.png',
            './assets/images/classroomThree.png',
        ],
        title:'Classrooms',
        text:'Our well-equipped classrooms are designed to provide a nurturing and stimulating learning environment. Each classroom is thoughtfully arranged to inspire creativity, curiosity, and engagement.'
    },
    {
        filter:'Library',
        imgsArr:[
            './assets/images/libraryZero.png',
            './assets/images/libraryOne.png',
            './assets/images/libraryTwo.png',
            './assets/images/libraryThree.png',
            './assets/images/libraryZero.png',
            './assets/images/libraryOne.png',
            './assets/images/libraryTwo.png',
            './assets/images/libraryThree.png'
        ],
        title:'Library',
        text:"Our expansive library is a treasure trove of books, fostering a love for reading and supporting students' literacy development."
    },
    {
        filter:'Science Lab',
        imgsArr:[
            './assets/images/sienceLabZero.png',
            './assets/images/siecneLabOne.png',
            './assets/images/siecneLabTwo.png',
            './assets/images/siceneLabThree.png',
            './assets/images/sienceLabZero.png',
            './assets/images/siecneLabOne.png',
            './assets/images/siecneLabTwo.png',
            './assets/images/siceneLabThree.png'
        ],
        title:'Science Lab',
        text:"Our hands-on science lab allows students to conduct experiments and explore scientific concepts in a fun and interactive way."
    },
    {
        filter:'Computer Lab',
        imgsArr:[
            './assets/images/computerLabZero.png',
            './assets/images/computerLabOne.png',
            './assets/images/computerLabTwo.png',
            './assets/images/computerLabThree.png',
            './assets/images/computerLabZero.png',
            './assets/images/computerLabOne.png',
            './assets/images/computerLabTwo.png',
            './assets/images/computerLabThree.png'
        ],
        title:'Computer Lab',
        text:"Equipped with age-appropriate technology, the computer lab enhances students' digital literacy and computational skills"
    },
    {
        filter:'Garden and Nature Area',
        imgsArr:[
            './assets/images/gardenZero.png',
            './assets/images/gardenOne.png',
            './assets/images/gardenTwo.png',
            './assets/images/gardenThree.png',
            './assets/images/gardenZero.png',
            './assets/images/gardenOne.png',
            './assets/images/gardenTwo.png',
            './assets/images/gardenThree.png'
        ],
        title:'Garden and Nature Area',
        text:"Our garden and nature area offer an opportunity for children to connect with nature and learn about plants and the environment."
    }
];
export {galleryCardData};